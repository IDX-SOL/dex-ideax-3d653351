import { useEffect, useState } from "react";
import { Link } from "react-router-dom";
import DemoGuideModal from "@/components/exchange-home/DemoGuideModal";
import {
  AFFILIATE_URL,
  CHANNEL_NEWS_URL,
  DOCS_EXCHANGE_URL,
  FUTURES_URL,
  GUIDE_URL,
  LAUNCHLAB_URL,
  MARKETS_URL,
  SITE_URL,
  SOCIAL_DISCORD_URL,
  SOCIAL_INSTAGRAM_URL,
  SOCIAL_LINKEDIN_URL,
  SOCIAL_TELEGRAM_URL,
  SOCIAL_X_URL,
  SOCIAL_YOUTUBE_URL,
  SWAP_URL,
  TOOLS_SITE_URL,
} from "@/config/exchange/urls";
import { withBasePath } from "@/utils/base-path";
import "@/styles/exchange-home.css";

export const DEMO_GUIDE_OPEN_EVENT = "idx-open-demo-guide";

export function ExchangeMarketingFooter() {
  const [demoGuideOpen, setDemoGuideOpen] = useState(false);

  useEffect(() => {
    const open = () => setDemoGuideOpen(true);
    window.addEventListener(DEMO_GUIDE_OPEN_EVENT, open);
    return () => window.removeEventListener(DEMO_GUIDE_OPEN_EVENT, open);
  }, []);

  return (
    <>
      <footer className="ex-footer">
        <div className="ex-wrap">
          <div className="ex-footer-bar">
            <Link to="/" className="ex-footer-brand">
              <picture>
                <source
                  srcSet={withBasePath("/exchange-home/logo.webp")}
                  type="image/webp"
                />
                <img
                  src={withBasePath("/exchange-home/logo.png")}
                  alt=""
                  width={28}
                  height={28}
                />
              </picture>
              <p className="ex-footer-name">IDX Exchange</p>
            </Link>
            <div className="ex-footer-social" aria-label="Social links">
              <a
                href={SOCIAL_X_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="X (Twitter)"
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"
                  />
                </svg>
              </a>
              <a
                href={SOCIAL_DISCORD_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Discord"
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037 12.3 12.3 0 0 0-.608 1.25 18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"
                  />
                </svg>
              </a>
              <a
                href={SOCIAL_TELEGRAM_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Telegram"
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"
                  />
                </svg>
              </a>
              <a
                href={SOCIAL_LINKEDIN_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"
                  />
                </svg>
              </a>
              <a
                href={SOCIAL_INSTAGRAM_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram"
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"
                  />
                </svg>
              </a>
              <a
                href={SOCIAL_YOUTUBE_URL}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="YouTube"
              >
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path
                    fill="currentColor"
                    d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"
                  />
                </svg>
              </a>
            </div>
            <Link className="ex-btn ex-btn-primary" to={FUTURES_URL}>
              Trade
            </Link>
          </div>

          <nav className="ex-footer-nav" aria-label="Footer">
            <Link to={MARKETS_URL}>Markets</Link>
            <Link to={SWAP_URL}>Swap</Link>
            <a href={LAUNCHLAB_URL}>Launch</a>
            <Link to={AFFILIATE_URL}>Refer</Link>
            <button
              type="button"
              className="ex-footer-nav-demo"
              onClick={() => setDemoGuideOpen(true)}
            >
              Demo
            </button>
            <a href={GUIDE_URL}>Guide</a>
            <a href={TOOLS_SITE_URL} target="_blank" rel="noopener noreferrer">
              Tools
            </a>
            <a href={DOCS_EXCHANGE_URL} target="_blank" rel="noopener noreferrer">
              Docs
            </a>
            <a href={CHANNEL_NEWS_URL} target="_blank" rel="noopener noreferrer">
              Crypto news
            </a>
          </nav>

          <p className="ex-footer-by">
            by{" "}
            <a href={SITE_URL} target="_blank" rel="noopener noreferrer">
              idxsolana.io
            </a>
          </p>
        </div>
      </footer>
      <DemoGuideModal
        open={demoGuideOpen}
        onClose={() => setDemoGuideOpen(false)}
      />
    </>
  );
}
